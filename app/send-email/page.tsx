...
const { data, error } = await resend.emails.send({
  ...
  return (
    <main className='max-w-2xl mx-auto px-4 py-12'>
      ...
      {/* <ResultDisplay data={result?.data} error={result?.error} loading={loading} title='Email Sent Successfully' /> */}
      ...
      <div className='mt-6 p-4 rounded-lg bg-[var(--muted)] border border-[var(--border)]'>
        ...
        <p className='text-sm text-[var(--muted-foreground)] mb-2'>
          Run <code className='bg-[var(--background)] px-1 rounded'>pnpm email:dev</code> to preview your email templates in the browser at{' '}
          <code className='bg-[var(--background)] px-1 rounded'>localhost:3000</code>.
        </p>
      </div>
